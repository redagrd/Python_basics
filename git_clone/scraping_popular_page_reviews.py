...
while True:
    for id in imdb_id:
        ...
        for review in reviews:
            ...
            # Récupérer l'URL de la Review
            try:
                review_url = review.find_element(By.CLASS_NAME, "title").find_element(By.TAG_NAME, "a").get_attribute('href')
            except:
                review_url = 'URL non trouvé'
            #récupérer la date de la review
            try:
                review_date = review.find_element(By.CLASS_NAME, "review-date").text.replace('\n','')
            except:
